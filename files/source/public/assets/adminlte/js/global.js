...
function initializeSideMenu() {
	...
	if (null === URLPrefix) {
		throw(new Error("document.body needs \"data-url-prefix\" attribute."));
	    return false;
	}
	...
	if (("login" == strPageURL) || ("forgotpassword" == strPageURL) || "profile" == strPageURL) {
		return;
	}
...
function getActiveDialogCSV() {
	var strActiveDialogCSV = "";

	if (document.body.getAttribute("data-active-dialog-csv")) {
		strActiveDialogCSV = document.body.getAttribute("data-active-dialog-csv");
	}
	
	return strActiveDialogCSV;
}

function setActiveDialogCSV(strActiveDialogCSV) {
	document.body.setAttribute("data-active-dialog-csv", strActiveDialogCSV);
}

function removeActiveDialog(modalId) {
	var strActiveDialogCSV = getActiveDialogCSV();
	var arrActiveDialogs = strActiveDialogCSV.split(",");
	var lengthDialogIds = arrActiveDialogs.length;

	var newActiveDialogCSV = "";

	for (var i = 0; i < lengthDialogIds; i++) {
		if (modalId == arrActiveDialogs[i]) {
			continue;
		}

		if (newActiveDialogCSV != "") {
			newActiveDialogCSV += ",";
		}

		newActiveDialogCSV += arrActiveDialogs[i];
	}

	setActiveDialogCSV(newActiveDialogCSV);
}

function appendActiveDialog(modalId) {
	var strActiveDialogCSV = getActiveDialogCSV();
	...
	strActiveDialogCSV += modalId;

	setActiveDialogCSV(strActiveDialogCSV);
}
...
function showDialog(modalId) {
	...
	$(modalDialog).on('hidden.bs.modal', function (e) {
	  removeActiveDialog(this.id);
	});

	appendActiveDialog(modalId);

	modalDialog.scrollTop = "0px";
}

function showErrorDialog(strMessage) {
	document.getElementById("pFormErrorText").innerHTML = strMessage;
	showDialog("modal-Error");
}
